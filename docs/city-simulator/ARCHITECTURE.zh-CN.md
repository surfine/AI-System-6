...
默认 `bonsai-renderer-canvas.js` 与懒加载 3D 后端共同实现。纯模块保留 64x32 的 2:1 投影、逆向拾取、
...
2D 精灵，3D 用同一几何构建实例网格。纹理构建读取保存的原创生成图稿与语义
配方，输出 48 张颜色纹理及独立线性玻璃/发光遮罩。颜色 alpha 保留透明语义；
遮罩 R 表示玻璃、G 表示夜间发光。UV 内缩半像素，图集关闭 mipmap 避免跨材质
采样。内置生图工具不暴露精确模型版本；运行时与普通构建均不调用生图、无需
模型 API 密钥。本次重制不改变模拟、命令或存档版本；本文不代表浏览器已验收
或已经部署。
