...
第一份完成的作品有两种刻意区分的尺度。**钟点稿**是短路线：开始一篇小作品，手写或显影成稿，持久保存，然后下载或分享。产品的完整长文路线仍是「项目硬盘 → 文件软盘 → 问题单 → 大纲 → 章节草稿 → 正文 → 审校台 → 项目光盘」。新控件必须让其中一条路线更容易、让已保存作品更安全，或让恢复继续更清楚；否则不进入当前 Beta。
