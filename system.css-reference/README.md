...
> AI System 6 local note: this directory is a visual reference and parts library, not a runtime dependency to import wholesale. The app's readable CSS lives in `styles.css`, and the startup runtime uses generated `styles.bundle.css` so the Floppy Budget can stay under `1,474,560` bytes.
