...
// Build the six-era 文字亮室 icon extension from the reviewed source SVGs.
// Lightroom is outside the canonical 56-object Finder family, so this builder
// owns its extension assets without changing the historical family manifests.
...
const sourceRoot = join(root, "tooling", "icon-generation", "lightroom-six-era");
...
const manifest = {
  schema: "ai-system-6-lightroom-icon-extension-v1",
  semanticIdentity: "a proof sheet under localized close inspection",
  sourceRoot: "tooling/icon-generation/lightroom-six-era",
  eras: {},
};

for (const [era, sizes] of Object.entries(sizesByEra)) {
  const sourceRelative = `tooling/icon-generation/lightroom-six-era/${era}.svg`;
...
writeFileSync(join(themesRoot, "lightroom-icon-extension.json"), `${JSON.stringify(manifest, null, 2)}\n`);
console.log("文字亮室: built six-era icon extension (Classic SVG + 5 raster eras + Liquid appearances)");
