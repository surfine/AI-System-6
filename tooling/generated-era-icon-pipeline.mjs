...
//
// Candidates stay under internal/evidence/drafts/icon-generation until a human approves them;
...
const workRoot = join(root, "internal/evidence/drafts/icon-generation");
...
function prepare() {
  ...
  for (const eraId of Object.keys(ERAS)) {
    ...
    writeFileSync(join(promptRoot, `${eraId}.json`), stableJson({
      ...
      manualInbox: `internal/evidence/drafts/icon-generation/inbox/${eraId}/<icon>.png`,
      records: eraRecords,
    }));
  }
...
function acceptCandidates() {
  ...
  const acceptedPath = join(root, "tooling/icon-generation/accepted-generated-icons.json");
  ...
  for (const [eraId, era] of Object.entries(accepted.eras)) {
    ...
    const archive = join(root, "apps/desktop/assets/themes", eraId, "icons/imagegen-source");
    ensureDir(archive);
  ...
  accepted.reviewedAgainst = [
    "internal/evidence/drafts/icon-generation/cross-era-candidate-board.png",
    "internal/evidence/drafts/icon-generation/cross-era-candidate-board-16px.png",
    "internal/evidence/drafts/icon-generation/key-fringe-before-after-light.png",
    "internal/evidence/drafts/icon-generation/key-fringe-before-after-dark.png",
    "internal/evidence/drafts/icon-generation/runtime-derived-16-light.png",
    "internal/evidence/drafts/icon-generation/runtime-derived-16-dark.png",
  ];
...
async function fringeReview() {
  const beforeRoot = resolve(option("before-root", join(root, "apps/desktop/assets/themes")));
  const columns = 2;
  const scale = 8;
...
async function runtime16Review() {
  const accepted = JSON.parse(readFileSync(join(root, "tooling/icon-generation/accepted-generated-icons.json"), "utf8"));
    ...
    for (let eraIndex = 0; eraIndex < Object.keys(ERAS).length; eraIndex += 1) {
      ...
      for (let index = 0; index < iconIds.length; index += 1) {
        ...
        const source = generatedIds.has(iconId)
          ...
          : join(root, "apps/desktop/assets/themes", eraId, "icons", `${iconId}-128.png`);
...
function printHelp() {
  ...
  node tooling/generated-era-icon-pipeline.mjs prepare
  node tooling/generated-era-icon-pipeline.mjs generate [--era aqua] [--icon finderApp] [--profile master|small|tiny] [--server http://127.0.0.1:4173]
  node tooling/generated-era-icon-pipeline.mjs import [--era aqua] [--icon finderApp]
  node tooling/generated-era-icon-pipeline.mjs board [--icons startupDisk,applications,fileFloppy,projectDisc]
  node tooling/generated-era-icon-pipeline.mjs fringe-review [--before-root /path/to/frozen/before]
  node tooling/generated-era-icon-pipeline.mjs runtime16-review
  node tooling/generated-era-icon-pipeline.mjs accept --approved
  node tooling/generated-era-icon-pipeline.mjs audit [--era aqua] [--partial]

Manual mode: run prepare, generate each PNG elsewhere from the exact prompt,
then place it at internal/evidence/drafts/icon-generation/inbox/<era>/<icon>.png and run import.`);
}
