...
/**
 * Write into one of this generator's own directories, re-creating it if it
 * left.
 *
 * The declared output directories are made before this script starts (see
 * tooling/build-preapp.mjs) and once more here. Neither is enough on its own:
 * measured 2026-09-20/21, four releases died inside this generator with ENOENT
 * on a path under this directory, eleven to forty seconds after it was made —
 * the copy of the tree the release was building in had already been proved
 * whole, so something removed the directory while the atlas was being encoded.
 * A short write is worth re-making a directory for; this writer is the one
 * place that can do it without knowing who took it.
 */
async function writeInto(directory, name, bytes) {
  await mkdir(directory, { recursive: true });
  await writeFile(path.join(directory, name), bytes);
}

function writeAsset(name, bytes) {
  return writeInto(assetsDir, name, bytes);
}
...
await writeAsset("atlas-metadata.json", `${JSON.stringify(metadata, null, 2)}\n`);
...
await writeInto(generatedDir, "bonsai-atlas.js", generated);
...
await writeAsset("provenance.json", `${JSON.stringify(provenance, null, 2)}\n`);
...
// atlas was drawn from.
{
  ...
  // reward at game scale, on grass, plus a junction of each network.
  {
    ...
    if (writesEvidence) {
      ...
      await writeInto(evidenceDir, "bonsai-facility-preview.png", sheetPng);
  ...
  if (writesEvidence) {
    ...
    await writeInto(evidenceDir, "bonsai-city-preview.png", previewPng);
