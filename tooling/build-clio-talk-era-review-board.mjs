...
const outDir = join(root, "internal/evidence/drafts/clio-talk-era-redraw");
...
for (let index = 0; index < eras.length; index += 1) {
  ...
  const large = await loadImage(join(desktopRoot, era.large));
  ...
  const regular = await loadImage(join(desktopRoot, era.regular || era.large));
  ctx.imageSmoothingEnabled = !era.pixelated;
  ctx.drawImage(regular, 606, y + 62, 64, 64);
  ctx.fillStyle = "#172033";
  ctx.fillRect(722, y + 62, 64, 64);
  const small = await loadImage(join(desktopRoot, era.small));
  ...
  if (era.appearances) {
    ...
    for (let appearanceIndex = 0; appearanceIndex < era.appearances.length; appearanceIndex += 1) {
      ...
      const image = await loadImage(join(desktopRoot, `assets/themes/liquid-glass/icons/assistant-64-${appearance}.png`));
