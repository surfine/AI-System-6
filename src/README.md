# src/ — AI System 6 server source

This directory is now the canonical server source for the main app. The
root `npm start` command runs `node src/server.js` after rebuilding the
browser bundle.

Older docs may describe `src/` as a mirror rewrite that runs on a separate
port. That phase is over. Treat `src/server.js`, `src/server/router.js`,
`src/server/routes/`, and `src/server/importers/` as the active server
...
`node src/server.js` also works, but it does not rebuild `app.bundle.js`.

`src/server/router.js` owns the route table and static file fallthrough.
...
- Keep shared server logic in `src/server/lib/` or the focused feature
...
- Reordering or removing the System 6 product details documented in
  `CLAUDE.md`.
- Migrating client code into `src/`. Browser code lives in root `app.js`
