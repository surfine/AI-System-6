...
test.assertIncludes(roles, "modelRolePatterns", "task kinds map to roles through one table");
...
test.assert(context.modelRoleForTaskKind("generate-outline") === "writer", "outline tasks use the Writer role");
test.assert(context.modelRoleForTaskKind("search-answer") === "researcher", "search tasks use the Researcher role");
test.assert(context.modelRoleForTaskKind("run-claim-check") === "critic", "claim checks use the Critic role");
test.assert(context.modelRoleForTaskKind("translate") === "utility", "translation uses the Utility role");

const normal = context.resolveModelRoleForTask("generate-outline");
test.assert(
...
const withoutRole = context.resolveModelRoleForTask("generate-outline");
test.assert(
...
const withRole = context.resolveModelRoleForTask("generate-outline");
test.assert(
...
const withFallback = context.resolveModelRoleForTask("generate-outline");
test.assert(
