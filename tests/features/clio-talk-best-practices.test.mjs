...
test.assertMatches(foundationCss, /\.clio-use-result-modal\[open\] \{[^}]*overflow: hidden/, "Use Result never scrolls its own dialog frame, so Cancel and Write cannot leave the screen");
test.assertMatches(foundationCss, /\.clio-use-result-panes \{[^}]*overflow: auto/, "Stacked on a phone, the panes region scrolls so the confirm row is never clipped off the bottom");
test.assertMatches(foundationCss, /@container \(min-width: 520px\) \{\s*\.clio-use-result-panes \{[^}]*grid-template-columns/, "A wide enough dialog puts the destinations beside their consequence instead of stacking four bands");
test.assertMatches(indexHtml, /id="clio-use-result-subject" class="clio-use-result-quote"/, "The reply appears as one quoted subject line, not as a tall centred column");
test.assertNotIncludes(chatMessages, 'subjectEl.textContent = compactClioTalkUseResultPreview', "The subject line collapses the reply to a single run of text");
test.assertIncludes(chatMessages, 'target.available ? "" : target.unavailableReason', "An available destination costs one row; only a destination the user cannot pick explains itself");
test.assertIncludes(chatMessages, 'purposeEl.textContent = selectedTarget?.help', "The detail pane carries the selected destination's purpose, so six rows do not repeat it six times");
test.assertMatches(indexHtml, /class="button-row finder-operation-actions">\s*<p id="clio-use-result-record"/, "The Run Record disclosure rides in the button row rather than adding its own band");
...
test.assert(
  /if \(!clioTalkTemporaryMode\)[\s\S]*downloadMarkdownAndBurnToProjectCd/.test(documentsChat)
    && /return downloadMarkdown\(markdown, file\.name\);/.test(documentsChat),
  "Exporting a Temporary Chat downloads Markdown without silently writing Project CD"
);
