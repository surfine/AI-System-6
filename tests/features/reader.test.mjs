...
test.assertIncludes(foundation, "--reader-meta-divider:", "the document header closes with one theme-owned rule");
...
test.assertIncludes(styles, ".reader-document-header {\n  margin-bottom: 1.6em;\n  padding-bottom: 10px;\n  border-bottom: var(--reader-meta-divider);", "title and provenance line share one header and one rule");
test.assertNotIncludes(`${foundation}\n${liquid}\n${styles}`, "--reader-section-marker", "Reader headings carry no side stripe");
test.assertIncludes(reader, "function readerBodyMarkdown(page)", "a body that repeats the document title as its own # heading starts after it");
test.assertIncludes(reader, 'meta.className = "reader-meta"', "one provenance line names site, author, date and length");
test.assertIncludes(reader, "function readerReadingLength(text)", "length is counted by characters for CJK and by words otherwise");
...
test.assertIncludes(styles, '.reader-pane[data-reader-state="loaded"]:not([data-reader-entry="open"]) > .reader-toolbar {\n  display: none;', "a loaded document gives the source-entry row back to the page on every screen");
test.assertIncludes(reader, "function setReaderPaneState(state)", "the pane states what it shows instead of layout guessing it from DocMap's availability");
test.assertNotIncludes(`${styles}\n${responsive}`, ".reader-pane:has(#reader-docmap-button", "a short source that cannot be mapped no longer brings the entry row back");
...
test.assertIncludes(styles, "@media (prefers-reduced-motion: reduce) {\n  .reader-selection-bar,", "the bar does not travel for readers who asked for less motion");
test.assertIncludes(reader, "readerSelectionReceipt = text;", "a clip confirms itself in the bar that made it");
