...
import { assertDocMapMetaphor, measureDocMapMetaphor } from "../../scripts/lib/docmap-metaphor-metrics.mjs";
...
for (const [theme, paths] of Object.entries(DOCMAP_ARTWORK)) {
  ...
  const measured = await measureDocMapMetaphor(join(root, paths.artwork), theme);
  let passes = true;
  try {
...
const board = read("scripts/build-icon-continuity-board.mjs");
...
test.assertIncludes(read("package.json"), '"build:icon-continuity-board"', "the board has its own command");
test.assertIncludes(read("package.json"), '"build:docmap-acceptance-board"', "DocMap before/after evidence has a focused command");
