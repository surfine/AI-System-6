// How Recipient Would Receive It has two layers:
// the default external card is short and copy-safe, while the backstage review
// keeps Creator/creator-facing guardrails out of the recipient handoff.
...
test.assertIncludes(zh, 'review_mingming_handoff: "若是接收者会怎么接"', "Chinese copy names the short-card handoff");
test.assertIncludes(zh, 'review_mingming_handoff_backstage: "交付后台审校"', "Chinese copy names the backstage review");
test.assertIncludes(en, 'review_mingming_handoff: "How Recipient Would Receive It"', "English copy names the short-card handoff");
...
test.assertIncludes(handoff, "用户可能是 Creator，也可能是接收者本人", "Prompt preserves the dual-user constraint");
...
test.assertIncludes(handoff, "## 给接收者看的 30 秒版", "Short card includes the 30-second recipient version");
...
test.assertNotIncludes(mingmingLens, "若是接收者会怎么接", "Existing Mingming lens is not polluted by handoff review behavior");
