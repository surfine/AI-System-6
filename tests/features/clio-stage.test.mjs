...
test.assertIncludes(slidesExport, "do not interview, ask questions, request confirmation", "Story Pass remains non-blocking and asks the user no setup questions");
...
test.assertIncludes(slidesExport, "Use <!-- _class: lead -->", "Generation prompt tells the model how to vary slide classes");
test.assertIncludes(slidesExport, "Use <!-- header: Section / Topic -->", "Generation prompt supports sparse breadcrumb headers");
test.assertIncludes(slidesExport, "Include concise speaker notes when useful with <!-- notes: ... -->", "Generated decks can include speaker notes without crowding slides");
test.assertIncludes(slidesExport, "Do not invent facts, claims, names, numbers, examples, images, sources, or conclusions not present in the source.", "Generation remains source-grounded");
test.assertIncludes(slidesExport, "Use a line containing only --- to separate slides.", "Generation still follows Marp slide separators");
...
test.assertIncludes(slidesExport, "每一页必须有真实可见内容；不要输出空白 slide", "repair prompt directly addresses empty-slide failures");
