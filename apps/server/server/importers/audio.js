...
const macosSpeechScriptPath =
  uniqueAppRootCandidates
    .map((candidate) => path.join(candidate, "tooling", "transcribe-audio-macos.swift"))
    .find((candidate) => fsSync.existsSync(candidate)) ||
  path.join(repoRoot, "tooling", "transcribe-audio-macos.swift");

const macosSpeechAnalyzerScriptPath =
  uniqueAppRootCandidates
    .map((candidate) => path.join(candidate, "tooling", "transcribe-audio-macos26.swift"))
    .find((candidate) => fsSync.existsSync(candidate)) ||
  path.join(repoRoot, "tooling", "transcribe-audio-macos26.swift");
