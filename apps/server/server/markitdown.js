...
// the same paths are found whether the process is launched from `apps/server/`
...
const markitdownAdapterPath =
  uniqueAppRootCandidates
    .map((candidate) => path.join(candidate, "tooling", "markitdown-adapter.py"))
    .find((candidate) => fsSync.existsSync(candidate)) ||
  path.join(repoRoot, "tooling", "markitdown-adapter.py");
