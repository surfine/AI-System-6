...
`icons/src/platinum-core-icons.json` 固定参考板、测量规则、来源版本与版权边界；`icons/platinum-core-icon-family.json` 记录每个合格对象的原型、原生文件、边界、颜色数量和 SHA-256。`tooling/build-platinum-core-icons.mjs` 是核心批次的确定性来源；完整的 `tooling/build-era-icons.mjs` 会在最后再次执行它，避免全量重建时退回旧核心图。
