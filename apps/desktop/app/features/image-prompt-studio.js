...
(() => {
  ...
  async function generate() {
    ...
    if (user && referenceDataUrl) {
      ...
      if (cloudActive) {
        // DeepSeek is text-only; a reference image would be silently ignored.
        setStatus("ips_ref_cloud_ignored", "Reference image is ignored on the cloud model; use a local vision model.");
      } else {
        user.content = [
  ...
  function render() {
    buildStudioWindow();
    if ($("ips-go") && $("ips-go").dataset.wired) return;
    const go = $("ips-go");
    if (go) {
      go.dataset.wired = "1";
