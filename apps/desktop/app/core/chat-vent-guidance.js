// Chat Vent Intake Guardrail: launch-day complaints and chat screenshots are
// author expression / creative material, not verified facts.

window.AISystem6ChatVent = (() => {
  const marker = "AI System 6 chat vent intake guardrail";
  ...
  function instruction({ language = currentLanguageCode() } = {}) {
    if (isZh(language)) {
      return [
        `${marker}: user venting and chat screenshots are author-expression material, not verified source facts.`,
        ...
        "聊天里别人说的话只能原样引用，并保留是谁、对谁说的；不要把某个人的话总结或转述成他的观点，也不要评价这个人。",
    ...
    return [
      ...
      "Quote other people's chat words verbatim, keeping who said them and to whom; do not summarise or paraphrase a person's words into their view, and do not characterise the person.",
