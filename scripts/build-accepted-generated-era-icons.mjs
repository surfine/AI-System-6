...
const ledgerPath = join(root, "scripts/icon-generation/accepted-generated-icons.json");
...
async function rebuildSprite(eraId, manifest, family) {
  ...
  for (let index = 0; index < entries.length; index += 1) {
    ...
    const image = await loadImage(join(root, "assets/themes", eraId, file));
    ctx.drawImage(image, (index % 8) * cellSize, Math.floor(index / 8) * cellSize, cellSize, cellSize);
  }
  writeFileSync(join(root, "assets/themes", eraId, `${eraId}-sprite.png`), canvas.toBuffer("image/png", { compressionLevel: 9 }));
}
...
export async function buildAcceptedGeneratedIcons(requestedEras = Object.keys(ledger.eras)) {
  ...
  for (const eraId of selected) {
    ...
    if (!era) continue;
    const themeDir = join(root, "assets/themes", eraId);
    ...
    family.generatedOverlayBuilder = "scripts/build-accepted-generated-era-icons.mjs";
    family.runtimeSize = 128;
    family.generatedAcceptanceLedger = "scripts/icon-generation/accepted-generated-icons.json";
