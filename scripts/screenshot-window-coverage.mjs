...
// chrome + default/empty state. Evidence lands in drafts/theme-coverage/
// (gitignored); the JSON index records each window's rect so a human can
// review the set.
//
// Usage:
//   node scripts/screenshot-window-coverage.mjs --theme platinum
//   node scripts/screenshot-window-coverage.mjs --theme platinum --out drafts/theme-coverage/windows
...
function parseArgs(argv) {
  const options = { theme: "platinum", out: "drafts/theme-coverage/windows" };
  for (let index = 0; index < argv.length; index += 1) {
    if (argv[index] === "--theme") options.theme = argv[++index];
    else if (argv[index] === "--out") options.out = argv[++index];
    else if (argv[index] === "--help") {
      console.log("Usage: node scripts/screenshot-window-coverage.mjs [--theme platinum] [--out drafts/theme-coverage/windows]");
      process.exit(0);
    }
  }
  return options;
