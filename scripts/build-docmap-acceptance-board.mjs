...
const outputDir = join(root, "drafts/era-icons");
...
for (let index = 0; index < eras.length; index += 1) {
  ...
  const image = await loadImage(join(root, relativePath));
...
const liquid16 = await loadImage(join(root, "assets/themes/liquid-glass/icons/docMap-16-default.png"));
