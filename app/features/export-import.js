...
function nativeObjectModelSpec() {
  return [
    { object: "ProjectDisk", nativeRole: "Root writing container; owns all user project data." },
    { object: "Documents", nativeRole: "Durable TeachText and saved chat files scoped to a Project Hard Disk." },
    { object: "PictureAlbum", nativeRole: "Project-scoped draft image album; compressed previews for writing and originals for Project CD delivery." },
    { object: "Reader", nativeRole: "Temporary reading surface; persists only saved pages or selected clips." },
    { object: "Scrapbook", nativeRole: "Selected evidence and idea clips with source contracts." },
    { object: "QuestionSheet", nativeRole: "User intent, original questions, objections, terms, and tone target." },
    { object: "Outline", nativeRole: "Editable structure that preserves user questions before drafting." },
    { object: "SectionDrafts", nativeRole: "Topic-level AI drafts tied to outline sections and clips." },
    { object: "TeachText", nativeRole: "Primary manuscript surface and insertion target." },
    { object: "ClaimCheck", nativeRole: "Source-trust layer using the shared source registry." },
    { object: "Translation", nativeRole: "Local bilingual writing layer with source/time/model metadata." },
    { object: "ProjectCD", nativeRole: "Final Markdown burned to the Project CD for the active Project Hard Disk." },
    { object: "Trash", nativeRole: "Recoverable deletion area scoped to the active Project Hard Disk." },
  ];
}
...
function buildNativeHandoff() {
  const project = getActiveProject();
  const projectFiles = getProjectFiles();
  const projectScraps = getProjectScraps();
  const projectRefs = projectReferences.filter((reference) => reference.projectId === activeProjectId);
  const projectCdHandoffs = getProjectCdItems();
  const sourceRegistry = buildProjectSourceRegistry();
  const progress = project && typeof getFlowProgress === "function" ? getFlowProgress(project) : null;

  return {
    format: "ai-system-6-native-handoff",
    formatVersion: 1,
    ...
    productDecision: {
      track: t(nativeProductDecision.trackKey),
      nativeTarget: t(nativeProductDecision.targetKey),
      prototypeBoundary: t(nativeProductDecision.boundaryKey),
      currentRuntime: nativeProductDecision.currentRuntime,
      nativeStatus: nativeProductDecision.nativeStatus,
    },
    nativeBoundary: {
      carryForward: [
        "Project Hard Disk scoped data model",
        "Writing object path",
        "sourceContract source IDs",
        "TeachText insertion metadata",
        "translation metadata",
        "Project CD burn semantics",
        "Chinese-first onboarding language",
      ],
      replaceLater: [
        "HTML window shell",
        "DOM textarea editing surface",
        "browser IndexedDB persistence",
        "server.js packaging shell",
        "web-only drag/drop behavior",
      ],
    },
    objectModel: nativeObjectModelSpec(),
    activeProject: project ? {
      id: project.id,
      name: project.name,
      counts: {
        documents: projectFiles.length,
        scraps: projectScraps.length,
        projectReferences: projectRefs.length,
        sourceRegistry: sourceRegistry.length,
        drafts: project.drafts?.length || 0,
        projectCdItems: projectCdHandoffs.length,
        trash: getProjectTrashItems().length,
      },
      writingPath: progress ? {
        topic: progress.states.topic,
        research: progress.states.research,
        outline: progress.states.outline,
        drafting: progress.states.drafting,
        check: progress.states.check,
        currentKey: progress.currentKey,
        complete: progress.complete,
      } : null,
      sourceKinds: countBy(sourceRegistry, (source) => source.kind),
      projectCdNames: projectCdHandoffs.map((item) => item.title),
    } : null,
    releasePosture: {
      webBeta: true,
      stableNativeApp: false,
      honestLabel: "beta workbench, not final native app",
      requiredBeforeNativeFreeze: [
        "one real Chinese writing handoff",
        "Project Hard Disk backup/restore proof on a fresh machine",
        "model failure and offline-state proof",
        "TeachText manuscript editing proof",
      ],
    },
  };
}

function nativeBriefMarkdown() {
  const handoff = buildNativeHandoff();
  const project = handoff.activeProject;
  const counts = project?.counts || {};
  const lines = currentLanguage === "zh"
    ? [
      "# AI System 6 Phase 27 Native 产品决定",
      "",
      "当前产品定位：中文 Web Beta，是 native-first 产品路线的验证工作台，不是最终 native app。",
      "",
      `版本：${formatAppVersion()}`,
      `产品轨道：${t(nativeProductDecision.trackKey)}`,
      `Native 目标：${t(nativeProductDecision.targetKey)}`,
      `原型边界：${t(nativeProductDecision.boundaryKey)}`,
      "",
      "必须继承到 native 的东西：项目硬盘可信边界、写作对象路径、来源 ID、TeachText 插入元数据、翻译元数据、Project CD 导出交付语义、中文上手路径。",
      "以后要替换的东西：HTML 窗口、DOM 编辑器、IndexedDB 外壳、server.js 打包壳、浏览器专属交互。",
      "",
      project
        ? `当前项目：${project.name}；文档 ${counts.documents || 0}，摘录 ${counts.scraps || 0}，草稿 ${counts.drafts || 0}，Project CD ${counts.projectCdItems || 0}。`
        : "当前项目：未挂载。",
    ]
    : [
      "# AI System 6 Phase 27 Native Product Decision",
      "",
      "Current posture: Chinese web beta as a native-first product workbench, not the final native app.",
      "",
      `Version: ${formatAppVersion()}`,
      `Product track: ${t(nativeProductDecision.trackKey)}`,
      `Native target: ${t(nativeProductDecision.targetKey)}`,
      `Prototype boundary: ${t(nativeProductDecision.boundaryKey)}`,
      "",
      "Carry forward: Project Hard Disk boundary, writing object path, source IDs, TeachText insertion metadata, translation metadata, Project CD handoff, Chinese onboarding.",
      "Replace later: HTML windows, DOM editor, IndexedDB shell, server.js packaging shell, browser-only interactions.",
      "",
      project
        ? `Active project: ${project.name}; documents ${counts.documents || 0}, scraps ${counts.scraps || 0}, drafts ${counts.drafts || 0}, Project CD items ${counts.projectCdItems || 0}.`
        : "Active project: none mounted.",
    ];

  return lines.join("\n");
}

async function copyNativeBrief() {
  await copyMarkdown(nativeBriefMarkdown());
  setStatus(t("native_brief_copied"));
}

function exportNativeHandoff() {
  downloadJsonFile(buildNativeHandoff(), "AI System 6 Native Handoff");
