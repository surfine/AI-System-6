...
function renderReaderTabs(project = getActiveProject()) {
  ...
  readerTabsEl.classList.remove("is-hidden");
  const label = document.createElement("span");
  label.className = "reader-saved-label";
  label.textContent = t("reader_tabs", tabs.length);
  readerTabsEl.append(label);

  tabs.forEach((tab, index) => {
    const item = document.createElement("div");
    item.className = "reader-tabs-item";
    item.draggable = true;
    item.dataset.documentTabId = tab.id;
    if (project && tab.id === project.activeDocumentTabIds?.reader) {
      item.classList.add("is-active");
    }
    item.addEventListener("dragstart", (event) => {
      event.dataTransfer.effectAllowed = "move";
      event.dataTransfer.setData("text/plain", tab.id);
      readerTabsEl.dataset.draggingTabId = tab.id;
      item.classList.add("is-dragging");
    });
    item.addEventListener("dragend", () => {
      delete readerTabsEl.dataset.draggingTabId;
      item.classList.remove("is-dragging");
      readerTabsEl.querySelectorAll(".is-drop-target").forEach((node) => node.classList.remove("is-drop-target"));
    });
    item.addEventListener("dragover", (event) => {
      const draggingId = readerTabsEl.dataset.draggingTabId;
    ...
    item.addEventListener("drop", (event) => {
      event.preventDefault();
      item.classList.remove("is-drop-target");
      const draggingId = event.dataTransfer.getData("text/plain") || readerTabsEl.dataset.draggingTabId;
      if (!draggingId || draggingId === tab.id) return;
      if (!moveDocumentTab("reader", draggingId, tab.id, project)) return;
      renderReaderTabs(project);
      saveDeskState();
    });

    const openButton = document.createElement("button");
    openButton.className = "btn reader-saved-open";
    openButton.innerHTML = [
      `<b>${index + 1}</b>`,
      `<span class="reader-tab-text">`,
      `<strong>${escapeHtml(tab.title || tab.url)}</strong>`,
      `<small>${escapeHtml(formatReaderTabSubtitle(tab))}</small>`,
      `</span>`,
    ].join("");
    openButton.title = [
      tab.title,
      tab.backing?.url || tab.backing?.fileName || tab.state?.source || tab.url || tab.fileName || tab.source,
    ].filter(Boolean).join("\n");
    ...
    removeButton.className = "btn reader-saved-remove";
    removeButton.type = "button";
    removeButton.textContent = "×";
    removeButton.setAttribute("aria-label", t("reader_close_tab"));
    removeButton.title = t("reader_close_tab");
    removeButton.addEventListener("click", () => {
      removeReaderTab(tab.id);
    });

    item.append(openButton, removeButton);
    readerTabsEl.append(item);
  });
}
...
function markReaderUrlOpened(page) {
  const project = getActiveProject();
  if (!project || !page?.url) return;
  if (typeof captureActiveReaderTabState === "function") captureActiveReaderTabState();
  upsertDocumentTab("reader", page.role || "source_view", {
