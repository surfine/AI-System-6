...
<!-- source-sha256: 3e2af719f4c4562d57804c2496c169cada95cfa87779cac481b998c59b477bec -->
...
## 公开测试版 1.0.55 — 2026-09-22 更新

- **现在有四面演示用项目硬盘，新增的两面都是完整项目。**
  `/go/m5ipad`《M5 iPad Pro：是 Mac 的防线，还是 AI 交互的起点？》与
  `/go/iphone17e`《iPhone 17e 浅粉色：其貌不扬，但很有料》和 `/go/dtk`、
  `/go/ipad1` 并列：问题单、六节大纲、六节草稿、正文、审校记录与素材谱系
  俱全，材料来自这台电脑上的 `~/M5 iPad Pro`、`~/M5-iPad-Pro`、微信导出
  记录，以及仓库里已有的 iPhone 17e 语料。加一块新盘＝登记表一行＋命令表
  里一个字符串：四条 `open-shared-disk-*` 命令合并成一张表加一个循环，
  所以这次加完，启动包余量比加之前还多。
- **演示盘窗口在手机上改用对话框、行内摘要会换行、并且跟着语言切换。**
  先量后改：420px 的窗口落在 left:148，在每个手机视口都跑出屏幕；607px 的
  行摘要在 372px 的窗口里溢到 880px；切换语言时整扇窗口留在旧语言里，而
  桌面上其他地方都换了。三处都已修好，并写进契约。
- **File 菜单的「打开演示用项目硬盘…」列出随应用发布的每一块盘。** 打开一个
  小窗口，逐行给出这块盘自己的主题句，并交给分享链接用的同一条导入路径；
  「写入项目硬盘」窗口的「项目备份」区里有同样的行。

## 公开测试版 1.0.55 — 2026-09-12

- 配色工作台里的配色，由官方产品场景自带的环境光照亮，所以你调出来的材料
  就是这件东西真正持有的材料。iPhone 17 Pro 与 Pro Max 合并为一个机型、
  两种姿态；iPhone Air、iPhone 17e、MacBook Neo 与 iPhone Duo 使用同一套
  逐部件配色，并可导出 USDZ。
- iPhone Duo 可以连续折叠：内外屏图片分开保存，原始壁纸一个按钮恢复，导出
  在本机完成——PNG、JPEG、WebP，或渲染出的折叠视频。
- Duo 同时是四个可以混搭的部件（边框、背板玻璃、铰链、相机平台），并配有
  正面、背面、左、右四个视角，以及自由转动场景相机的能力——刚上的那个颜色，
  看得到。
- 每个独立应用现在只有一个可分享地址：`/go/<app-id>`。裸的应用目录、发布出去
  的跳转文件和服务端自己的直连路由都已撤下，安装器会在安装 release 的同一次
  事务里退役旧的发布路径，所以没人能再分享一个站点已经不再提供的地址。
- 公网站点可以承载 MCP 访客桥：Pages 提供与桌面端相同的工具契约，签发指名
  某一张桌面的邀请，并把访客执行器放在 Durable Object 后面。
- 发布门禁与实际出货内容重新对齐：prompt 契约跟随应用真正携带的 70 条记录，
  README 的量表引用门禁量到的载荷，集成账本把这条发布分支记为已并入。

## 公开测试版 1.0.54 — 2026-09-11

- CMF Studio 新增 iPhone 18 Pro 与 iPhone Duo，并使用 Apple 授权的交互素材。
- iPhone Duo 支持连续折叠、内外屏独立图片，以及本地 WebM 折叠视频导出。
- MacBook Neo 的打开姿态也使用同一套连续铰链交互。
...
## 公开测试版 1.0.52 — 2026-09-04

这一版兑现城市建造的承诺，也让模型路由不再替你二选一。

Micropolis 与 Bonsai City 现在说同一种仪表语言。需求柱同时出现在细节条与
工具栏底部，点它打开评价或城市报告，消息需要你注意时会闪烁；手机上紧凑的小柱
保留，大面板让位。Bonsai City 也守住一座城市模拟该有的承诺：镜头回到你离开的
位置、预算逐月记住、机场和高速公路按游戏年份解锁而不是一次全给、四种穹顶城市
家族以不同人口与解锁年份生长、十四种灾害（暴动、堆芯熔毁、飓风、空难等）共享
同一个与天气联动的世界。存档升到 v4 格式并带迁移链，任何旧城市都能打开，一行
也不丢。

终末地终端按证据该有的样子作答：先编号出处卡，再结论与留白。检索仍由服务器
确定性完成，而答案可以由你自己浏览器里的 LM Studio 模型书写——或按你的选择走
云端——不再由某一侧替另一侧决定。

ClioTalk 不再逼你在两件有用的事之间挑一件。同时打开联网搜索并附图：带引用的
检索答案会和图片一起出现在同一轮回复里，引用来源仍然一键可查。

LM Studio 路径优先使用原生 v1 API——有状态对话、模型加载事件、请求内上下文
长度——只有模型需要 v1 装不下的形态时才依次回退到 Responses API 与
OpenAI 兼容接口。

### 1.0.52 巡礼构建 — 2026-09-05

这次跟进构建把发布做成一场巡礼：收集在第一个 1.0.52 stamp 之后仍在继续工作的
各 lane 与 agent 的成果，作为一个版本落地。

- 需求表不再等截屏。Micropolis 就是 SimCity——GPL 开源且已内置——仪表绘制代码
  就能精确回答 M7/M8：绿/蓝/黄纯色柱、黑描边、`#e6e6ff` 底板。两个游戏像素格
  也修掉一处裁切误读：过去拍的是地形，现在守住了调色板与需求表的承诺。
- 桌面双向说 MCP：访客 agent 可以读当前项目、把审阅作为可采用的运行回执交回；
  桌面也可以去问其它 MCP 服务器，答案落在文件软盘上。
- 写作工作室的暗房从手稿真正的主人那里重绘，不再把页面画到碰巧打开的窗口上；
  新契约拒绝任何没有运行者的门禁，除非豁免写明理由。
- 安装后的应用支持主屏幕长按直达 ClioTalk、Quick Draft 与 Lightroom。
- 终末地档案按恢复上线的实时路由重建任务语料：332 个任务、26,413 行对白都带
  v1.5 戳，而不是给旧文本贴新版本号。

贡献逐 lane 记在 git 历史里，量色合并还带 agent 联合署名；仍未收尾的事项
（SC2000 面板布局与图标、ClioTalk 双模态真实 smoke、整机手机横屏清扫）继续
开放，记录在 1.0.52 closeout。

## Public Beta 1.0.55 - 2026-09-22 更新

- **File 菜单的「打开演示用项目硬盘…」** 会列出随应用发布的两块盘：弹出的小窗口显示每块盘自己的主题句，并把打开动作交给共享链接用的同一条导入路径；「写入项目硬盘」窗口的「项目备份」区里有同一份列表，「说明」文件夹里也多了讲这件事的那一篇。
- **手工导入的项目硬盘现在打得开。** `internal/` 里两份源盘都漂移了：它们自报四个文件、实际装着十一个，摘要又是照着那份旧计数算的，所以在导入窗口里选中文件只会得到「backup content has changed」。规则现在只有一处（生成器、新的 `npm run disks:restamp` 与一条直接问应用自己的校验器的契约共用）。
- **准入成本付掉了第一块。** 十六条「只是打开一个窗口」的命令与四篇说明文档，各自从几个文件里近似的重复行变成一张表：这一版新增了一行 File 菜单、一个说明入口与一条窗口注册记录，启动包反而少了 1155 字节。

## Public Beta 1.0.53 - 2026-09-05

- 语义检索不再依赖 AI 服务商是否提供 embedding 模型。当 DeepSeek 作为聊天模型、或没有连接本地模型时，一个小型多语言 embedding 模型会在浏览器里安静运行，检索继续按语义排序，而不是悄悄退回关键词。云端与本地检索的体验一致。
- 本地 LM Studio 集成不再堆积重复的已加载实例。重新加载模型会先卸载旧副本，服务器不再把同一个 embedding 模型报成加载了十次。
- 经服务器提供的页面可以显式指定云端或本地路由，让模型修复与载荷路由在公开站点上保持一致。
...
- **One More Tune 多了第二种玩法，题库也更锋利。** Keynote Relay 问的是苹果请上台的那些人：每轮六次交接、提示扣分，答案只留在服务器上。音乐题库现在记录每张卡能问什么、用哪条声音线索，以及哪些标签不能再当答案——一轮里不会重复同一个主题，也不会拿近似答案凑数。
- **官网与 README 都放上七帧外观画面**，Liquid Glass 图标家族也在两个被接受的构建阶段里，用同一个名字指明它的 Golden Gate Beta 1 目标。
- **演示用项目硬盘坐进了启动磁盘**，就在你自己的那块「项目硬盘」旁边：一行、打开同一个面板（与 File 菜单同一入口），而且它们**仍然不是卷**——在打开并变成普通项目之前，什么都不会挂载。

- **演示用项目硬盘变成了一个文件夹，而且打开文件夹不再下载整批正文。** 随应用发布的三十四块盘现在住在启动磁盘的一页 Finder 里——项目数、图标／列表视图、选中、简介、双击打开——不再是模块自建的小窗；File 菜单那一行打开的是同一个页面，而某块盘的「简介」用的是作者写在问题单上的那句话。清单独立成 16 KB 的模块，所以打开文件夹不再把背后 3.7 MB 的正文一起拉下来（1:134）；某一块盘的备份只在那一块被打开时加载。启动包从 2,946,313 变成 2,948,224 字节，距「两张软盘」那条线还剩 896 字节，所以下一个要占启动包的功能必须回收而不是新增。
- **One More Tune 多了第三种玩法「下一幕 / Next Act」。** 一轮十道纯文字题，问一句发布会台词当时在做什么、接着进入哪一章，取自发布会台词研究包：四个选项、不限时、一题一分、可以跳过。答案、解释与来源都留在服务器上，直到提交回来；跳过的那一题照样打开它的说明；而同一句话的两道题（WWDC23 与 WWDC24 的 One more thing、2022 与 2023 的「下楼」）永远不相邻——否则第一题的说明就把第二题的答案说出去了。这一版没有声音，窗口自己也说清了原因：这些台词背后的录音尚未逐句审听，所以声音版继续关着。

## 公开测试版 1.0.55 — 2026-09-23 更新

- **装到主屏幕上以后，顶上那一条带子属于应用自己。** 菜单栏现在在**每一个触摸面、两个方向**上都长进状态栏区域，而不只是手机竖屏：实测在 iPad 上，八套外观的菜单标题一直被画在时钟与电池底下——因为预留那条带子的规则只写在一条「手机竖屏」的媒体查询里。带子现在是平的、不透明的，而不是半透明材质：模糊盖在半透明栏上，等于桌面在透出来。八套外观 × 四个面共 32 个组合，改之前有 22 个是错的，现在是 0。
- **One More Tune 的六色环变成了一个圆盘。** 它原本是两张互不相关的画——背后一个静态圆、前面一个 82px 的环——而进度标记不论有没有在播都画在 22%。现在外圈弧读的是**这一段试听自己的时钟**（不是它出自的那条影片），转盘只在有声音时转、暂停时保留相位，六个楔子只点亮一个：这张卡所属的那一代。揭晓的命名搬到唱片下的唱套上；舞台太矮放不下时（横屏手机那条 104px 的带子）它回到揭晓卡片里，两者永远不会同时出现。外圈是 `progressbar`，位置未知时不设值——未知不是 0。
- **被拒绝的播放现在会说出来。** iOS 上每一条商店试听都走媒体元素而不是解码器，而那条路径原本把引擎的答复扔掉了：一次根本没开始的播放照样被记成听过一次，界面还画成正在播的样子。现在它会把这笔账退回去、把这一题标成坏题（可以跳过，而不是白丢一分），并说出唯一能解决它的动作；元素也会记住自己曾被拒绝，下一次手势会重新上膛。
- **三十四块演示盘全部对着聊天导出重读了一遍。** 其中十四块此前从未引用过它，这一轮补进了材料，并把导出、日期、原话与落点写进各自的素材谱系；另有五块只写了边界声明，做了同样的处理；其余十五块是核对已有引用。其中一条经不起核对的引注被**标了出来**，而不是悄悄改掉，之后也找到了来源：airbattery 的「1 小时 37 分」是作者本人的实测，2024-08-08 发出，收在为这块盘做准备的备忘录里，盘上现在照实这样写。每一块盘都留有一条书面结论，包括「查过、无新增」。
- **七块演示盘改写成了频道自己的专栏。** iPhone 4S Demo 机、MagSafe 原型、iPhone 16 工程机、iPad Air 4 工程机、CDMA iPhone 4、陶瓷 Apple Watch 与 iPhone 11 Pro Max，每一块都对照它出自的那期节目和频道后来的勘误核过，再按作者的论点和口吻重写，整块盘从问题单到审校记录保持一致。每块都由作者逐一确认；标题跟随已发布的节目，其余各盘在之后的测试版里陆续跟上。
- **NeXTSTEP 作为第八套外观正式发布。** 它此前一直停在 `releaseReady: false` 后面；按 3.3 版界面规范返工之后——主菜单、左侧滚动条、分三区读的 Dock，以及这套外观映射到的每个对象都有自己的图标——它和另外七套一样出现在 Special 菜单、控制面板和启动选择里。
- **Mac OS X 各时代的 Finder 窗口，照它们当年的样子打开。** Aqua 是工具栏型窗口；Snow Leopard、Yosemite、Big Sur 与 Liquid Glass 另加一条边栏：「个人收藏」（应用程序、文稿）与「位置」（启动磁盘、项目、挂载中的文字盘、废纸篓），每一行都在同一个窗口里导航。Big Sur 与 Liquid Glass 把标题并进单行工具栏。经典时代保持空间式 Finder；在手机上，每个时代都保留单栏窗口。
- **Big Sur 不再在里面穿着 System 6。** Big Sur 窗口里仍画着 1988 年样式的信息条、编辑框、滚动通道和视图按钮，现在都听这个时代的；ClioChart 的投影图例与工作台也换成 Big Sur 自己的字体。其中一部分出在一个解析陷阱上：声明在 `:root` 上、指向正文界面字体的字体令牌，会在任何外观来得及改它之前就解析成 Chicago；现在由使用处携带回退值，并有一条门禁拒绝这种写法。
- **演示文稿的八个时代，现在像各自那一年的演示软件。** 1988 年是投影胶片的外框，NeXTSTEP 是黑色斜面面板加 Times 标题，Platinum 是黑底红楔，Aqua 是渐变，Snow Leopard 是影棚光晕，三种现代扁平风格在字重与对齐上各不相同——而不是八套配色套在同一个版式上。打印保留每种版式自己的对齐，深色页保持深色，时代装饰也不再挤占幻灯片的正文空间。
- **One More Tune 的 41 道历史题经过三轮作者审阅，进入轮换。** 每一题的正确答案都放回它引用的 Newsroom 页面核对过（41 题中 40 题成立；不成立的那一题现在同时引用了支持文章），同场图注能为之辩护的两个干扰项换掉了，另有四道「正确答案最长或最短」能被猜中的题改了措辞；题库契约从此拒绝长度泄底的正确答案。替未审题目占位的练习卡已经移除。
- **官网的外观菜单列出的，就是这一版发布的八套外观。** Big Sur 作为第七套发布后从没进过那个菜单，而页面文案自己在数外观；现在有一条门禁拿菜单与已发布集合逐一对账。NeXTSTEP 以「岔路」而不是「一站」的身份加入：有自己的菜单分组、列表行和一幕，不在 1988 到 2026 的时间线上——把一张 NeXT 桌面放在 System 6 和 Platinum 之间，等于冒认一条它没有的 Mac 血脉。每个时代的画面都重新拍过，平面风格的几个时代改为无损存储，逐像素准确，体积反而更小。官网本身也为普通读者重写了一遍：一段来历、附来源的「往事」长文、在真实截图上移动镜头的导览，以及第一次来的人会问的问题。同一个页面也说清了一块盘靠什么地址旅行——README 有这条，官网此前没有。

## 公开测试版 1.0.55 — 2026-09-23 第二次更新

- **ClioProject 按当初的设计重做了一遍。** 第一版做偏了：标题没盖 id 的节直接消失（DTK 盘的六节一节都没画出来），没法挂自己的任务，单击就开窗口，每张卡片都带一个日期框，也没有日历。现在计划图从左往右走，用 MacProject 自己的图例——路线各站是里程碑，节和你挂的任务是任务——各节从「节稿」分出、在手稿汇合，做完的收进项目实际所在位置身后的一叠。每张卡片说一句项目记录里本来就有的事实（字数、引了几条剪报、是否已进手稿）；选中后在一个简介栏里编辑；单击选中、双击打开，双击一节会打开节稿并停在那一节。状态栏回答卡在哪、下一步、离交付还有几天。「显示」菜单切到「所有磁盘」：只读的月历，画交付日、刻盘日和字数变化的日子。
- **已经写到一半的项目，第一次打开计划时只问一次。** 它列出记录里已有成果的站和节，默认勾上，你留下哪些就勾哪些，也可以都不勾；无论怎么答都记住。
- **磁盘上的风险菱形改为读日期。** 交付日一周内或已过而交付那站未勾，或者写了日期的任务已过期，才会标记；只认读得出唯一一天的写法，其余仍是你的原话。原来「两周没动」的规则删掉了。
- **计划图没有多占启动软盘。** 它的样式挪进按需加载的样式包，菜单随模块一起到。
