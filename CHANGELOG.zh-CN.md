...
<!-- source-sha256: b24a35462babf0ffcb96ff853e6dcc8b7b85bac9cec5360069ec775934caa0f3 -->
...
最后一个收敛日把盆景城市带进公开测试版。窗口围绕地图重建：命令收进 Macintosh
菜单，工具轨变成紧凑工具板，小地图与叠层成为常驻仪表，触控也有了明确的「手形平移」
工具。它下面是目前最大的一轮城市扩展：128 方格地图、交通与公用事业、预算与债券、
公共服务与灾害、scenario 与地形编辑、原创声音与四季美术、读取同一份确定性快照的
两种视图，以及只用合成 fixture 验证、全程在浏览器本地运行的 clean-room `.sc2`
导入和导出。

写作桌也补上了合并暴露出的四处安静断点：状态现在落在真正握有光标的写作面；联动
预览重绘时不再重置读者的位置；句子聚焦重新可用；「查找／替换」、气球帮助与弹出
菜单都留在它们服务的窗口旁边。ClioTalk 的欢迎文案会跟随所选时代的字体；图片提示词
工作室也找回了那张曾让整扇窗口塌成零宽的样式表。

**Build 20260830.1 — 2026-08-30。** 收敛后的补发构建：把版本真正发到了每一个目标，
也补上了收敛留下的缝隙。

并发的窗口不再互相踩踏桌上的记录。另一扇窗口里已经变过的记录会拒绝过期写入，而不是
悄悄覆盖；删除和写入一样有栅栏；只读窗口会保留它所属的可编辑窗口，而不是凭空消失。

Theme Lab 的菜单行现在还原出它测量的原生截图：Snow Leopard 的选中行是实测的 20px，
Aqua 是 21px，16px 的文字行高成为每个纪元都要作答的周期 token，而不是少数几个纪元
继承的兜底。Yosemite 的搜索框也吃到同一套纪律，终于够到之前差一点的地板。

首次启动以 ClioTalk 为前门，外观、视觉与 Theme Lab 整板基线都围绕这个契约重新抓取，
而不是旧的欢迎盘桌面。六套新图标家族——盆景城市、DOOM、Micropolis、OpenTTD、
文字亮室与图片提示词工作室——随可复现的构建器一起发布它们的纪元美术；公开源码快照
现在能从全新克隆完整自测：atlas 构建器把维护者证据留在私有侧，公开树预算也跟上了
随版本发布的资产。

本构建以 1.0.50（build 20260830.1）发布到 GitHub 公开快照与 Mac Beta v1.0.50-beta.4、
VPS 线上主机与 Cloudflare Pages；Pages 部署改为临时副本执行，密封的发布产物保持
字节不变。

**Build 20260831.3 — 2026-08-31。** 剩下的几条 lane 一起落地，桌面上能看到的路
比之前更完整。

第一分钟取代了原来的开场长文。陌生人的卡片——现在叫“先看 30 秒”——在任何
模型状态下都会播放 30 秒离线放映；看完后会给出“自己逛逛”和“三分钟写一条”
两扇门，后者打开快速草稿直接开写。中途退出视为没有完成引导。

ClioProject 成为一扇真正的窗口：计划板从路线与提纲实时推出节点和箭头，用粗体
标出挡在交接前面的那条链，日期原样保留，已完成的任务自动折起。To Do 待办以
桌面附件身份到来，跨项目的下一步行动绝不会被悄悄丢掉。画片簿获得单张图片的
云端选项，本地仍是默认，由云端读取时会明说；有風險的项目磁盘戴上调色盘的菱形
标记，并且只依据产品已有的实据定义。

文字亮室守住了自己的对象。外来对象在关门那一刻被释放，用只读徽章报出自己的
名字，所有合成来源读的都是对象自己的文本；只读对象下可以试看，develop 仍然
拒绝。File 在阅读视图获得页面设置与打印，Adjust 的撤销显影是一次 ⌘Z。听稿桌
获得伙伴会话（只有标记和一条笔记——没有名字、没有模型），依赖视觉的发现给出
视觉提示出口，交付菜单可导出 SRT 或分镜表，时间线明确标注是估算。

盆景城市的三种检视模式升级为完整演示：带健康、教育与失业趋势的年龄金字塔，
跟随时代曲线的十部门产业构成，以及一张四邻城的罗盘卡片——贸易跟着你真正修好
的连线走。ClioStage 的讲台放得进窗口了：演示视图整体缩小到下限后改为滚动；
解析出来却从未上屏的指令终于抵达画框——页眉、页脚与页码按逐页覆盖渲染，讲者
备注也按 Markdown 渲染。

两条 bugfix lane 在同一构建里收尾。被拒绝的桌面记录保存不会在任何表面上谎称
已保存：冲突存续期间，正文状态胶囊与桌面单元格都显示未保存，之后成功的提交会
通过一次刷新把每个表面恢复，纸张也会重绘那份被保住的文本。Aqua 与 Snow
Leopard 的横向滚动条拇指则回到各自的方向性配方——蓝色渐变的条纹与胶囊端帽沿
条带宽向排布，而不是顺着条长铺开。

外观校验改用 token 表而非像素来守住中间四个纪元：同一轮里以 2,075 个探针点把
计算值与 Classic 对比，任何注入纪元表的几何或 token 漂移都会按名字快速失败。

本构建以 1.0.50（build 20260831.3）发布到 GitHub 公开快照与 Mac Beta
v1.0.50-beta.5、VPS 线上主机与 Cloudflare Pages。

## Public Beta 1.0.51 - 2026-09-02

这一版关于兑现产品早已作出的承诺。

其中最直白的一条是：在 Aqua 与 Snow Leopard 外观下，把光标放进写作面，那一页
就被擦掉了。字还在——能选中、拼写检查还在画线——但没有被画出来，因为这两个时代
在承载墨迹的那一层之上刷了一层不透明的聚焦底色。Liquid Glass 把同样的文字冲淡，
而六个外观里，选中一行都会把它盖住。同一个缺陷类的六处实例，是靠读图层顺序找到
的，不是靠比对颜色值。现在有一道契约在五个写作面 × 六个外观 × 聚焦与失焦 ×
三种宽度上解析真实层叠，任何可能盖住作者自己文字的填充、滤镜或内阴影都会让它变红。

写作路线学会了把稿子交出去。稿子导出为真正的 `.docx`，文件里的页面与屏幕上的
页面来自同一份文本和同一份页面设定；结构自检不过就拒绝写盘，并说出它读到的那个
事实。审校台的意见作为真正的 Word 批注锚在各自的句子上，而正文与「不带审校」的
导出逐字节相同——意见是交给人的建议，不是背着人做的修改。三个内置技能负责判断
这份工作该用什么载体、以及一份文稿或一张表格欠读者什么，而从不碰句子本身。

两个对象不再谎报自己。以这套外观命名的 Liquid Glass 滑块此前没有任何样式，完全
由浏览器绘制；现在它是玻璃，窗口背后也会折射而不是染色。在手机上，打开问题单
曾经会让写作者一秒后落在手稿里——因为伴随窗口带着焦点打开，而单前台的屏幕把位置
让给了它；现在路线是有意地交接屏幕，横竖屏都如此。

验证本身也在「安慰」而非「检查」的地方被修好了。像素网六分钟里大半在等它根本
拍不到的窗口，只覆盖 33 格中的 20 格；现在 99 秒覆盖 33 格。合并校验器对着一个
非合并提交会说「合并内容已校验」。启动测试垫片会为任何被问到的 id 凭空造一个
元素，因此曾报告 76 个窗口里 72 个健康，而它们真正的构建过程一次都没跑过——诚实
的垫片上线后找到的第一个缺陷，就是 Time Machine 的「剪藏 + 翻译」：它从不剪藏，
却把责任推给翻译。
